    ...
    <div class="bg-white border border-gray-100 rounded-xl overflow-hidden">
        ...
        <div class="px-6 py-4 border-b border-gray-100 flex items-center gap-3">
            <div class="w-8 h-8 rounded-lg bg-[#009990]/10 flex items-center justify-center">
                <svg class="w-4 h-4 text-[#009990]" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75"/></svg>
            </div>
            <div>
                <h2 class="text-sm font-medium text-gray-800">Nuevo empleado</h2>
                <p class="text-xs text-gray-400">Completa los datos y asigna los permisos</p>
            </div>
        </div>

        <form method="POST" action="{{ route('employees.store') }}" class="p-6 flex flex-col gap-5">
            @csrf
            ...
            {{-- Teléfono --}}
            <div>
                <label class="block text-xs font-medium text-gray-600 mb-1.5" for="phone">Teléfono <span class="text-gray-300">(opcional)</span></label>
                <input
            ...
            <div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <label class="flex items-start gap-3 p-4 border border-gray-200 rounded-xl cursor-pointer hover:border-[#009990]/40 transition has-[:checked]:border-[#009990] has-[:checked]:bg-[#009990]/05">
                        <input type="checkbox" name="can_tickets" value="1" {{ old('can_tickets') ? 'checked' : '' }} class="mt-0.5 accent-[#009990]">
                        <div>
                    ...
                    <label class="flex items-start gap-3 p-4 border border-gray-200 rounded-xl cursor-pointer hover:border-[#074799]/40 transition has-[:checked]:border-[#074799] has-[:checked]:bg-[#074799]/05">
                        <input type="checkbox" name="can_access" value="1" {{ old('can_access') ? 'checked' : '' }} class="mt-0.5 accent-[#074799]">
                        <div>
